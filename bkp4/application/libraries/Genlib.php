...
class Genlib {
    ...
    public function superOnly() {
        ...
        if (empty($_SESSION['userid']) || (empty($_SESSION['level']) || $_SESSION['level'] !== 1)) {
            redirect(base_url());
        }
    }
    ...
    public function OwnerOnly() {
        //prevent access if user is not logged in or role is not "Super"
        if (empty($_SESSION['userid']) || (empty($_SESSION['level'])  ||  $_SESSION['level'] != 2)) {
            redirect(base_url());
        }
    }

    public function AccountantOnly() {
        //prevent access if user is not logged in or role is not "Super"
        if (empty($_SESSION['userid']) || (empty($_SESSION['level'])  ||  $_SESSION['level'] != 3)) {
            redirect(base_url());
        }
    }

    public function AccountantOnly2() {
        //prevent access if user is not logged in or role is not "Super"
        if (empty($_SESSION['userid']) || (empty($_SESSION['level'])  ||  $_SESSION['level'] != 4)) {
            redirect(base_url());
        }
    }
    ...
    public function EmployeeOnly() {
        //prevent access if user is not logged in or role is not "Super"
        if (empty($_SESSION['userid']) || (empty($_SESSION['level'])  ||  $_SESSION['level'] != 5)) {
            redirect(base_url());
        }
    }
