...
//set working days
        if(!$this->Main->isExist1('average_monthdays','company_id', $this->session->userdata('companyarray')[$companyindex]['id'] )){

        	$this->db->insert('average_monthdays', ['company_id'=>$this->session->userdata('companyarray')[$companyindex]['id']]);
 ...
 //setting bonus
          ...
          if(!$this->Main->isExist1('bonus','company', $this->session->userdata('companyarray')[$companyindex]['id'] )){

        	$this->db->insert('bonus', ['company'=>$this->session->userdata('companyarray')[$companyindex]['id']]);
		...
		$data['bfs'] = $this->Main->all3('benefits_funds','is_active', '1','is_deleted', '0' ,'company_id', $this->session->userdata('companyarray')[$companyindex]['id']);
		...
		$values['view'] = $this->load->view('accountant/settings', $data, TRUE);
...
if($this->session->userdata('level') != 5){
		...
		$values['view'] = $this->load->view('accountant/payroll_set', $data, TRUE);
	...
	{  
		...
		$values['view'] = $this->load->view('accountant/givesalary', $data, TRUE);
