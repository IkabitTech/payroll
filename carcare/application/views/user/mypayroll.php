...
<div class="container notprint">
  ...
  <span></span><a class="btn btn-default " href="<?= base_url().'payroll/salary_slip/'.$payroll ?>" > PRINT SALARY SLIP</a>
</div>
...
<h2> PAYROLL (<?=   date('M Y', strtotime($this->Main->mycolumn1('payroll_rec', 'name', 'id', $payroll)))   ?>) -<?= $allnames ?><strong></strong></h2>
...
<table class="table  table-hover table-striped table-bordered table-responsive printable" id='confirmed18' style="width:100%; ">
  <thead>
    <tr style="height: 0px">
      ...
      <th scope="col">BASIC (TZS)</th>
       ...
       <?php if($this->Main->isExist2('payroll_records_arrears', 'company',$co,'payroll',$payroll)){ ?>

      <th scope="col">ARREARS</th>
    <?php } ?>
      <th scope="col">DAY OVERTIME</th>
      <th scope="col">TOTAL PAY</th>
         <?php if($this->Main->isExist1('have_night_overtime', 'company',$co)){ ?>
      ...
      <th scope="col">TOTAL PAY</th>
               <?php if($this->Main->isExist2('payroll_records_allowance', 'company',$co,'payroll',$payroll)){
     ...
     <?php if($this->Main->isExist2('payroll_records_bonus', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){ ?>

      <th scope="col">Bonus</th>
    <?php } ?>

     <?php if($this->Main->isExist2('payroll_records_employer_deductions', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){ ?>
    ...
    <?php if($this->Main->isExist2('payroll_records_benefits_fund', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){
  ...
  <tbody class="" >
   ...
   <?php if($this->Main->isExist2('payrollrecords', 'company',$co,'payroll',$payroll)){
    ...
    <tr >
      ...
      <td ><?= $mybasic ?></td>
       ...
       <?php if($this->Main->isExist2('payroll_records_arrears', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){ ?>

      <td><?= $this->Main->mycolumn3('payroll_records_arrears','amount', 'payroll',$payroll,'employee',$p->employee,'company', $this->session->userdata('companyarray')['company_id']) ?></td> 
    <?php } ?>

      <td ><?= $p->overtime_day_hours ?></td>
      <td ><?= $p->o_day ?></td>
  <?php if($this->Main->isExist1('have_night_overtime', 'company',$this->session->userdata('companyarray')['company_id'])){ ?>
      ...
      <?php if($this->Main->isExist2('payroll_records_allowance', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){
     ...
     <?php if($this->Main->isExist2('payroll_records_bonus', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){ ?>
      ...
      <td><?= $this->Main->mycolumn3('payroll_records_bonus','amount', 'payroll',$payroll,'employee',$p->employee,'company', $this->session->userdata('companyarray')['company_id']) ?></td> 
    ...
    <?php } ?>
     ...
     <?php if($this->Main->isExist2('payroll_records_employer_deductions', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){ ?>
       ...
       <td><?= $this->Main->mycolumn3('payroll_records_employer_deductions','amount', 'payroll',$payroll,'employee',$p->employee,'company', $this->session->userdata('companyarray')['company_id']) ; $mydeductions += $this->Main->mycolumn3('payroll_records_employer_deductions','amount', 'payroll',$payroll,'employee',$p->employee,'company', $this->session->userdata('companyarray')['company_id'])  ?></td> 

    <?php } ?>

       <td ><?= number_format($gross) ?></td> 
       <td ><?= number_format($this->Main->payecalculations($payroll, $co, $p->employee, (int)$gross, (float)str_replace(array(',',' '), '', $mybasic),$p->basic)['taxable'] ,2)?></td> 
       <td ><?= $total_loans ; $mydeductions+= $total_loans   ?></td> 
       <td ><?= number_format($this->Main->payecalculations($payroll, $co, $p->employee, $gross, (float)str_replace(array(',',' '), '', $mybasic),$p->basic)['PAYE'],2); $mydeductions += $this->Main->payecalculations($payroll, $co, $p->employee, $gross, (float)str_replace(array(',',' '), '', $mybasic),$p->basic)['PAYE'] ?> </td> 

       <td ><?= number_format($this->Main->payecalculations($payroll, $co, $p->employee, $gross, (float)str_replace(array(',',' '), '', $mybasic),$p->basic)['social'],2); $mydeductions+=$this->Main->payecalculations($payroll, $co, $p->employee, $gross, (float)str_replace(array(',',' '), '', $mybasic),$p->basic)['social'] ?> </td> 
    ...
    <?php if($this->Main->isExist2('payroll_records_benefits_fund', 'company',$this->session->userdata('companyarray')['company_id'],'payroll',$payroll)){
              ...
              <?php }else{ ?>
               ...
               <?php }}}?>  
                ...
                <td ><?= number_format($this->Main->calculatedfund($payroll, $co,  $p->employee, $f->fund,  $gross, (float)str_replace(array(',',' '), '', $mybasic),$p->basic)['topup'])  ?></td>              
