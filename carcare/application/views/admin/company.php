...
<div class="modal fade in" id="imagelogo<?= $c->id ?>" >
                <div class="modal-dialog">
                    <div class="modal-content">
                      
                        <div class="modal-body">
                     <p class="lead"><?= $c->name ?></p>
                            <img id="imgtodisplay<?= $c->id ?>" class="media-object" src="<?= base_url() ?>assets/images/company_logo/<?= $c->logo ?>" onerror="this.onerror=null;this.src='http://payroll.vipajijobs.com/assets/images/logo.png';" style="height:200px;" >
                         ...
                         <form action="" method="POST" role="form" enctype="multipart/form-data">
                                ...
                                <input class="hidden" type="number" name="id" readonly="true" value="<?= $c->id ?>" required="true">
...
<!--id='${multipartFilePath}' name='${multipartFilePath}'-->
    ...
    <input type="file" style="visibility:hidden; width: 1px;" name="image" onchange="$(this).parent().find('span').html($(this).val().replace('C:\\fakepath\\', '')); readURL(this,'imgtodisplay<?= $c->id ?>')" accept="image/png" required="false">

                                        <!-- Chrome security returns 'C:\fakepath\'  -->
      <input class="col-md-5 btn btn-default" type="button" value="Change Image.." "="" onclick="$(this).parent().find('input[type=file]').click();">
    ...
    <span class="badge badge-important"></span>
                                ...
                                <button name='send_image' type="submit" class="btn btn-primary ">Submit</button>
                            </form>
    ...
    <tr >
      ...
      <td ><?= $c->status ?></td>
      <td onclick="$('#imagelogo<?= $c->id ?>').modal('show')" >

      <img src="<?= base_url() ?>assets/images/company_logo/<?= $c->logo ?>" onerror="this.onerror=null;this.src='<?= base_url() ?>assets/images/logo.png';" style="height:30px" ></td>

      <td><div id='branch_<?= $c->id ?>'><?php $mng = 1;
        if(!empty($branches)){

       foreach ($branches as $key => $value) {

if($this->Main->mycolumn1('client_branch', 'company', 'id',$value['id']) == $c->id){
          if($value['active'] == 1){$status = 'Active';}else{ $status = 'Inactive';}
       ?> 

       
       <p><strong><?= $mng ?>: </strong> <?= $value['name'] ?> , <a><?= $status ?></a></p>

        <?php  $mng++; }}} ?></div></td>


      <td><div id='manager_<?= $c->id ?>'><?php $mng = 1;
       ...
       <p><strong><?= $mng ?>: </strong> <a href="#"><strong><?= $value['username'] ?></strong></a>  <?= $value['full_name'] ?> , <a><?= $status ?></a></p>

        <?php  $mng++; }} ?></div></td>

      <td id=''><div id='maccountant_<?= $c->id ?>'><?php $mng = 1;
       foreach ($admins as $key => $value) {
        if(($value['level'] == 3 || $value['level'] == 4 ) && $this->Main->isExist2('companyadmins', 'client',$c->id,'admin', $value['id'])){
          if($value['active'] == 1){$status = 'Active';}else{ $status = 'Inactive';}
       ?> 
...
'<?= $c->company_url ?>'
       ...
       <td>
        <a href='<?= base_url() ?>company/edit/<?=$companyindex?>?company=<?= $c->id ?>' class="label label-primary"> Edit </a> 
        <a href='#managers<?= $c->id ?>' class="label label-primary" data-toggle='modal' > Manager </a>  
           <a href='#maccountants<?= $c->id ?>' class="label label-primary" data-toggle='modal'> Main Accountants </a> 
           <a href='#baccountants<?= $c->id ?>' class="label label-primary"  data-toggle='modal'>  Branch  Accountant </a> 
        <a href='#branches<?= $c->id ?>' class="label label-primary" data-toggle='modal'>  Branch </a> 
