   ...
   <?php if($this->Main->isExist2('payrollrecords', 'company',$this->session->userdata('companyarray')[$companyindex]['id'],'payroll',$payroll)){
    ...
    <tr >
      ...
      <td ><?= $allnames ?></br>
        ...
        <a style='margin-left:20px; margin :5px' class='badge badge-primary' href="#" onclick="salary_slip(<?= $p->employee ?>,<?= $payroll ?>,<?= $companyindex ?>)" >Send an Email to View Slip</a></br>

       <a  href="<?= base_url() ?>records/salary_slip/<?= $payroll ?>/<?= $p->employee ?>/<?= $companyindex ?>" class="badge badge-primary" style='background:#F07057; margin :5px' >View Slip and Print</a>
...
<div class="modal fade " id="writeemail" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="false">
...
<input id= 'payroll' class='hidden'>
<input id= 'co' class='hidden' value="<?= $companyindex ?>">
<input id= 'user' class='hidden'>
</form></div>
<div class="col-md-12"><a class="btn btn-default " onclick="salary_slip2()" >Submit Value</a></div>
