...
<a  class=" label label-primary " href="<?= base_url() ?>actions/createpayroll/<?= $this->session->userdata('companyarray')[$companyindex]['id'] ?>/<?= $payroll ?>"  rel="nofollow" style = "background-color: #F07057; color:white;display: inline-block;font-size:12px">Loading All Employee Who are not in List</a>
...
onclick="
      ...
      $('#editpossibles').modal('show');
          // $('#myvalue').focus();
       $('#myvalue').val(<?= $p->overtime_day_hours ?>);
  ...
  <?php if($this->Main->isExist1('have_night_overtime', 'company',$this->session->userdata('companyarray')[$companyindex]['id'])){ ?>
      ...
      $('#editpossibles').modal('show');
          // $('#myvalue').focus();
       $('#myvalue').val(<?= $p->overtime_night_hours ?>);
...
onclick="
      ...
      $('#editpossibles').modal('show');
          // $('#myvalue').focus();
       $('#myvalue').val(<?= $p->overtime_holiday_hours ?>);
...
<!--Controls-->
    ...
    <?php if($this->session->userdata('level') == 3 && $this->Main->mycolumn1('payroll_rec','confirmed','id',$payroll) != 1){ ?>
