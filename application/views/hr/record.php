...
$this->db->where('company', $this->session->userdata('companyarray')[$companyindex]['id']);
 ...
 $all4 = $this->Main->all_order('payrollrecords',['payroll'=> $payroll,'company'=>$this->session->userdata('companyarray')[$companyindex]['id']]);
  if(!empty($all4)){foreach ($all4 as $key => $a2) {

     $my[$a2['employee']][$a2['value_id']] =  $a2['value'] ;  
...
$i++;
 ...
 ?>
    ...
    <?php 
      ...
      <td><?= isset($my[$a['employee']][$value['id']])?$my[$a['employee']][$value['id']]:''  ?></td>
