...
<?php if(((int)$this->session->userdata('level') == 10 || (int)$this->session->userdata('level') == 1) ){ ?>



<a  class=" label label-primary " href="#" onclick=' $("#loadedentry").html(" Loading...");$.get("<?= base_url() ?>actions10/createpayroll/<?= $this->session->userdata("companyarray")[$companyindex]["id"] ?>/<?= $payroll ?>",{},function(r){ $("#loadedentry").html("");addednumberemployees(r)  });'  rel="nofollow" style = "background-color: #F07057; color:white;display: inline-block;font-size:12px">Loading</a>
...
<a  class=" label label-primary " data-toggle="modal" href="#uploadExcel" style = "background-color: #F07057; color:white;display: inline-block;font-size:12px">Upload New/Udated Filled Excel</a>

 <a  class=" label label-danger " data-toggle="modal" href="#deletemyemployeeall" style = "">Remove/Clear this Payroll Data</a>
...
<table class="table   table-hover table-striped table-bordered applydt" style="width: 100%">
  <thead>
    <tr >
      <th scope="col">#</th>
...
$i++;
 ...
 ?>
    ...
    <?php $mydata = $this->Main->all2('payroll_columns','payroll',$payroll,'company', $this->session->userdata('companyarray')[$companyindex]['id']);
      ...
      <td><?= $this->Main->mycolumn3('payrollrecords','value','payroll', $payroll,'employee', $a['employee'],'value_id', $value['id']) ?></td>
...
}).done(function(rd){console.log(rd)}).fail(function(error){console.log(error)});
